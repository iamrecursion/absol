...
% This section will:
% \begin{itemize}
%     \item \textbf{Conclude with Requirements:} The section will conclude with a high-level requirements specification for the project.
% \end{itemize}
...
\begin{itemize}
    \item \textbf{Specify Language Syntax:} The metalanguage must provide a flexible and intuitive way of specifying the syntax of the DSL.
    \item \textbf{Specify Language Semantics:} The metalanguage must provide an intuitive method for the specification of language semantics in the general case, and also provide special-case semantics for things that cannot be proved by the proof mechanism.
    \item \textbf{Data Types:} The metalanguage must provide a useful set of data-types to allow for useful computation to be performed. 
    These must include integer and floating point types, as well as list and matrix container types.
    \item \textbf{Function Calls:} The metalanguage must provide the ability to define procedures callable from host languages, and subroutines callable internally.
    ...
    \item \textbf{Data Traversal:} The metalanguage should provide mechanisms for traversing the data types it defines, so as to avoid the need to allow general recursion.
    \item \textbf{Ground Truth Semantics:} The metalanguage should provide a way to specify ground-truth semantics for non-terminals. 
    These are things that can be trivially assumed to terminate by the proof mechanism.
    \item \textbf{Extension Mechanisms:} The metalanguage should provide a set of `language features' that can be imported into scope for use by the DSL designer. 
    These should provide useful language constructs (e.g. non-terminals, special semantics) to aid in the construction of both syntax and semantics.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Parse Metaspec:} The metacompiler must be capable of both lexing and parsing a metaspec file into an appropriate AST data structure. 
    \item \textbf{Verify Language Construction:} The metacompiler must be capable of verifying that all used non-terminal symbols are defined, and that no non-terminal is defined more than once.
    These are pre-requisites for the verification engine.
    \item \textbf{Verify Semantic Form:} The metacompiler must be capable of verifying that all semantic rules match their appropriate forms.
    This includes all types of special semantic rules, as well as the user-defined operational-style semantics.
    \item \textbf{Verify Semantic Guards:} The metacompiler must be capable of verifying that there is a complete set of guards for the user-defined semantics in every production.
    \item \textbf{Generation of Verification Reports:} In the case where the metacompiler is unable to verify an input language, it should generate detailed diagnostics that indicate the reason(s) why the language failed to verify.
    ...
    \item \textbf{Defer Typechecking:} The metacompiler does not deal with semantic type checking, as type holes cannot be inferred at the language level. 
    \item \textbf{Prevention of Arbitrary Recursion:} The metacompiler should check that languages do not enable arbitrary recursion over data, as this would make termination impossible to show at the language level. 
    \item \textbf{Extensibility:} The metacompiler should be designed in such a fashion that it can easily be extended in the future to accommodate further developments on the way to productisation. 
...
\begin{itemize}
    \item \textbf{DSL Compiler Generation:} While an important part of `productising' this toolchain, the generation of the DSL compiler from the metaspec AST is out-of-scope.
    ...
    \item \textbf{Full Semantic Guard Checking:} As discussed in Section~\ref{sub:guard_completeness_checking}, it is possible to develop a sophisticated mechanism for checking the completeness of the guards.
