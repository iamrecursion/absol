...
[(sat, gs) | 
    ...
    \begin{itemize}
        \item \citet{Mernik:2005:DDL:1118890.1118892} recognises that an external DSL can act as a portable store for this information that can be reused across multiple systems. 
...
Examples of such context-sensitive language features are ``declaration of identifiers before use'' and ``well-typedness of expressions'' \citep{mosses2005action}.
Such syntax can either be defined by attribute grammars (which can be specified in EBNF, discussed in Section~\ref{ssub:extended_backus_naur_form}), or as static program semantics (semantics defined by program structure) \citep{mosses2005action}. 
...
Program semantics are modelled by semantic functions: expressions that map the abstract syntax of a program to the semantic entity that represents the program behaviour \citep{mosses2005action}:
\begin{equation*}
    f_s(\text{abstract syntax}) \mathgoesto \text{semantics}
\end{equation*}

\citet{mosses2005action} defines two semantic functions to be equivalent at a given level of decomposition if two phrases of program syntax are interchangeable without altering the meaning of the program. 
...
While such semantics are simpler to reason about, not all program expressions can be represented as compositional semantics \citep{mosses2005action}.\\
...
There are multiple kinds of operational semantics, but the two most common semantic frameworks are \textit{Structured Operational Semantics (SOS)} and \textit{Natural Semantics}.\\

Structured operational semantics (also known as small-step operational semantics) specify transition relations (semantic functions) that are characterised by phrase transitions depending only on the transitions of one or more of its sub-phrases \citep{Zhang:2004:SSD:981009.981013}:
\begin{itemize}
...
The hope is that this brings additional modularity to the semantics to allow for easy composition, but this has seen little uptake in practice.
...
There have been multiple hybrid frameworks, but one of the most studied is the Action Semantics framework designed by \citet{mosses2005action}, and used in multiple projects such as \citet{brown1992actress} and \citet{diehl1996semantics}.
...
\begin{itemize}
    ...
    \item It provides direct support for the specification of control flow, data flow, scoping and concurrent communication \citep{mosses2005action,doh2001composing}.
...
This restriction is imposed by the set of computational constructs originally envisioned by \citet{mosses2005action}, with the system proving hard to extend.\\
...
While there has been much research into the subject, with proposals such as Modular Monadic Semantics, Modular Action Semantics and even Modular Monadic Action Semantics, none of these frameworks have seen particular use \citep{wansbrough1997modular,mosses2005action,Zhang:2004:SSD:981009.981013,Mosses:2009:CS:1596486.1596489,mosses2001varieties}.
Peter Mosses has written extensively about modular semantic frameworks as embodied by his PLanCompS project, but such semantic models that he terms `Funcons' have seen little uptake in practice \cite{Mosses:2009:CS:1596486.1596489,Churchill:2014:RCS:2577080.2577099,Binsbergen:2016:TSC:2892664.2893464}.
...
While \citet{Hoare:1969:ABC:363235.363259} and \citet{manna1974axiomatic} propose a generic axiomatic framework for reasoning about computer programs, the best framework is the well-specified semantics of the programming language in which the program is written.
...
While, in general, it is an undecidable problem to determine if an arbitrary Turing-Complete program is correct, it is possible to restrict the set of allowable operations to a set that can be shown to terminate \citep{walther1994proving}.
...
This would, hopefully, allow a demonstration that the language semantics can be defined inductively on the language syntax.\\
...
The literature having established that it is possible to specify a language with provably correct semantics, some mechanisms needs to exist to translate such programs into an executable form.
...
Furthermore it is not as performant as some traditional parsers due to the additional state maintained for error reporting, as evidenced by the existence of Attoparsec \citep{gummelt2011hindsight}. \\
